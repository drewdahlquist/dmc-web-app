      ...
      <Box width="medium">
        <Formik
          ...
          validationSchema={Yup.object({
            machine: Yup.string("Select the HTP machine")
              .oneOf(["SRP", "SRP2", "RootBot"], "Select a valid machine")
              .required("Required"),
            experimentName: Yup.string("Enter the experiment name")
              .min(4, "Must be 4 characters or more")
              .max(64, "Must be 64 characters or less")
              .required("Required"),
            positions: Yup.number("Select number of positions")
              .min(1, "Must have at least 1 position")
              .max(8, "Must have at most 8 positions")
              .required("Required"),
            frequency: Yup.number("Enter the frequency of photos")
              .min(1, "Must be at least 1 hour in between photos")
              .max(72, "Must be at most 72 hours in between photos")
              .required("Required"),
            endDate: Yup.date("Enter the date you want the experiment to end")
              .min(new Date(), "Must be later than now")
              .max(
                new Date(new Date().getTime() + 365 * 24 * 60 * 60 * 1000),
                "Must not be later than a year from now"
              )
              .required("Required"),
